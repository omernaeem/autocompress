...
def write_schedule(layer_name, sparsity, epoch):
    ...
    schedule.write("    epochs: ["+str(epoch+i)+"]" + "\n")
...
def run_distiller():
    ...
    if(running_first_layer):
        resumepath = 'plain20best.pth.tar'
    else:
    ...
    os.system(
        "python3 compress_classifier.py -a plain20_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000000000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml  --resume " + resumepath + " --epochs 1 --effective-train-size " + str(
            train_size) + " --effective-valid-size " + str(valid_size) + " --effective-test-size " + str(
...
def run_distiller_final():
    ...
    os.system(
        "python3 compress_classifier.py -a plain20_cifar "+ DATASET_PATH + " -p 30 -j=4 --lr=0.01 --batch=256 --out-dir logs/plain20/ --compress=schedules/lrsched.yaml --resume lastlayerbest.pth.tar --epochs " + str(
            PRUNE_EPOCHS) + " --gpus 0" )
