...
maxtarget_currentlayer = -999

running_first_layer = True

epochs_done = 0

layer_names = []
layer_sizes = []
layer_sizes_compressed = []
layer_sparsities = []
...
def write_final_schedule(starting_epoch):
    ...
    schedule.write("    arch: 'resnet56_cifar'\n")
...
def write_schedule(layer_name, sparsity, epoch):
    ...
    for i in range(0, li):
        ...
        schedule.write("    class: 'L1RankedStructureParameterPruner'\n")
        schedule.write("    group_type: Filters" + "\n")
        schedule.write("    desired_sparsity: " + str(float(prune_levels[layer_names[i]])) + "\n")
        schedule.write("    weights: ['" + layer_names[i] + "']\n")
    ...
    schedule.write("    epochs: ["+str(epoch+epochs_done)+"]" + "\n")
...
def read_model_details():
    ...
    for line in f:
        ...
        layer_names.append(data[0])
        layer_sizes.append(data[1])
        layer_sizes_compressed.append(data[1])
        layer_sparsities.append(data[2])
        network_size = network_size + int(data[1])
        layer_zeros_storage_reqs.append(0)
...
def read_sparsities():
    ...
    for line in f:
        data = line.split(',')
        layer_sizes_compressed[i]=(data[1])
...
def run_distiller_summary():
    os.system(
        "python3 compress_classifier.py -a resnet56_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.00000001 --out-dir logs/resnet56/  --epochs 1 --resume resnet56best.pth.tar --summary sparsity --gpus 0")
...
def run_distiller():
    ...
    global running_first_layer

    if(running_first_layer):
        resumepath = 'resnet56best.pth.tar'
    else:
        resumepath = 'lastlayerbest.pth.tar'



    os.system(
        "python3 compress_classifier.py -a resnet56_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.01 --out-dir logs/agp75/ --compress=schedules/temp.yaml  --resume " + resumepath + " --epochs 1 --effective-train-size " + str(
            train_size) + " --effective-valid-size " + str(valid_size) + " --effective-test-size " + str(
...
def run_distiller_final():
    # os.system(
    #     "python3 compress_classifier.py -a resnet56_cifar "+ DATASET_PATH + " -p 30 -j=4 --lr=0.1 --batch=256 --out-dir logs/resnet56/ --compress=schedules/final.yaml --resume resnet56best.pth.tar --epochs " + str(
    #         PRUNE_EPOCHS) + " --gpus 0" )

    os.system(
        "python3 compress_classifier.py -a resnet56_cifar "+ DATASET_PATH + " -p 30 -j=4 --lr=0.01 --batch=256 --out-dir logs/resnet56/ --compress=schedules/lrsched.yaml --resume lastlayerbest.pth.tar --epochs " + str(
            PRUNE_EPOCHS) + " --gpus 0" )
...
def black_box_function(prune_level):
    ...
    if not points_searched.__contains__(rounded_prune_level):
        ...
        #sparsity = float(layer_sparsities[li]) #sparsity comes as zero for net thinning case
        density = (float(layer_sizes_compressed[li])/float(layer_sizes[li]))
    ...
    global maxtarget_currentlayer

    if target > maxtarget_currentlayer:
        maxtarget_currentlayer = target
        checkpoint_path = read_checkpoint_path()
        os.system("mv " + checkpoint_path + "/checkpoint.pth.tar currentlayerbest.pth.tar")
...
for name in layer_names:
    ...
    if name == 'module.conv1.weight' or name == 'module.fc.weight' or str(name).__contains__('conv2') or str(name).__contains__('down'):
        prune_levels[name] = '0'
        continue
    ...
    # black_box_function(**{'prune_level':0.5})
    ...
    running_first_layer = False

    os.system("mv currentlayerbest.pth.tar lastlayerbest.pth.tar")
