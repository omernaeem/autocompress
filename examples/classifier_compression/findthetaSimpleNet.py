...
def run_distiller_summary():
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/  --epochs 1 --resume logs/baseline/2019.04.01-203750/best.pth.tar --summary sparsity")
...
def run_distiller_quant():
    ...
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume " + checkpoint_path + "/checkpoint.pth.tar --epochs 1 --effective-train-size " + str(
...
def run_distiller():
    ...
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume logs/baseline/2019.04.01-203750/best.pth.tar --epochs 1 --effective-train-size " + str(
            train_size) + " --effective-valid-size " + str(valid_size) + " --effective-test-size " + str(
...
def run_distiller_final():
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar "+ DATASET_PATH + " -p 30 -j=4 --lr=0.002 --out-dir logs/agp75/ --compress=schedules/final.yaml --resume logs/baseline/2019.04.01-203750/best.pth.tar --epochs " + str(
            PRUNE_EPOCHS))
...
def run_distiller_final_quant():
    checkpoint_path = read_checkpoint_path()
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0015 --out-dir logs/agp75/ --compress=schedules/final_quant.yaml --resume " + checkpoint_path + "/checkpoint.pth.tar --epochs " + str(
            QUANT_EPOCHS) + " --gpus 0")
