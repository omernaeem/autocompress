...
PRUNE_EPOCHS = 1
QUANT_EPOCHS = 1
layer_biases = [64, 192, 384, 256, 256, 4096, 4096, 1000]
...
def write_final_schedule(starting_epoch):
    ...
    for name in layer_names:
        ...
        schedule.write("    final_sparsity : " + str(float(prune_levels[name]) + 0.001) + "\n")
    ...
    for name in layer_names:
        ...
        schedule.write("    ending_epoch: " + str(starting_epoch + 2) + "\n")
        schedule.write("    frequency: 1\n")
        i = i + 1
...
def run_distiller_final_test():
    ...
    os.system("rm -rf logs/agp75")
    os.system(
        "python3 compress_classifier.py -a alexnet " + DATASET_PATH + " -p 30 -j=4 --lr=0.001 --out-dir logs/agp75/ --compress=schedules/final.yaml --pretrained --epochs 1 --effective-train-size " + str(
            train_size) + " --effective-valid-size " + str(valid_size) + " --effective-test-size " + str(
            EFFECTIVE_TEST_SIZE_PRUNE) )
    # top1 = read_top1()
    # sparsity = read_sparsity()
    # if ((read_done() == 1)):
    #     return top1, sparsity
...
def black_box_function(prune_level, bitwidth):
    ...
    print('{0}, {1:0.3f}, {2:0.3f}, {3:0.3f}, {4:0.3f}, {5:0.3f}, {6:0.3f}, {7}, {8}, {9}, {10}\n'.format(iteration,
                                                                                                          target,
                                                                                                          prune_level,
                                                                                                          top1_prune,
                                                                                                          top1,
...
prune_levels = {'features.module.0.weight': '0.21', 'features.module.3.weight': '0.36', 'features.module.6.weight': '0.43', 'features.module.8.weight': '0.32', 'features.module.10.weight': '0.31', 'classifier.1.weight': '0.96', 'classifier.4.weight': '0.95', 'classifier.6.weight': '0.74'}
bitwidths = {'features.module.0.weight': '8', 'features.module.3.weight': '6', 'features.module.6.weight': '4', 'features.module.8.weight': '4', 'features.module.10.weight': '3', 'classifier.1.weight': '3', 'classifier.4.weight': '3', 'classifier.6.weight': '3'}
