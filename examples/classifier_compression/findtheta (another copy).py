...
def write_final_schedule_quant(epoch):
    ...
    for name in layer_names:
        layer_name = name[name.find(".") + 1:name.find(".w")]  # from . to .weight
...
def run_distiller_summary():
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume logs/baseline/2019.04.01-203750/best.pth.tar --epochs 1 --summary sparsity")
...
def run_distiller_quant():
    ...
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume " + checkpoint_path + "/checkpoint.pth.tar --epochs 1 --effective-train-size " + str(
            train_size) + " --gpus 1 >/dev/null 2>&1")
    # os.system("python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume " + checkpoint_path + "/checkpoint.pth.tar --epochs 1 --effective-train-size " + str(train_size) + " --gpus 1")
    top1 = read_top1()
...
def run_distiller():
    ...
    train_size = (mini_batch / training_dataset_size) * nbatches
    os.system(
        'python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume logs/baseline/2019.04.01-203750/best.pth.tar --epochs 1 --effective-train-size ' + str(
            train_size) + ' >/dev/null 2>&1')
    # os.system('python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume logs/baseline/2019.04.01-203750/best.pth.tar --epochs 1 --effective-train-size ' + str( train_size))
    top1 = read_top1()
    sparsity = read_sparsity()
    if ((read_done() == 1)):
        return top1, sparsity
...
def run_distiller_final():
    os.system(
        'python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.002 --out-dir logs/agp75/ --compress=schedules/final.yaml --resume logs/baseline/2019.04.01-203750/best.pth.tar --epochs '+str(PRUNE_EPOCHS))


def run_distiller_final_quant():
    checkpoint_path = read_checkpoint_path()
    os.system(
        "python3 compress_classifier.py -a simplenet_cifar ../../../data.cifar10 -p 30 -j=4 --lr=0.002 --out-dir logs/agp75/ --compress=schedules/final_quant.yaml --resume " + checkpoint_path + "/best.pth.tar --epochs "+str(QUANT_EPOCHS)+ " --gpus 1")
...
def black_box_function(prune_level, bitwidth):
    ...
    progressf.write('{0}, {1:0.3f}, {2:0.3f}, {3:0.3f}, {4:0.3f}, {5:0.3f}, {6}\n'.format(iteration,
                                                                                          target,
                                                                                          prune_level,
                                                                                          top1_prune,
                                                                                          top1,
