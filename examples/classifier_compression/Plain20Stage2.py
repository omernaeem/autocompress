...
RESUME_PATH = "25combplain20.pth.tar"
layer_biases = [16, 5, 9, 6, 15, 7, 13, 21, 24, 16, 25, 32, 25, 45, 64, 57, 55, 31, 20, 10]
...
def write_schedule_quant(layer_name, bitwidth, epoch):
    ...
    if QUANTIZE_ACTIVATIONS == 1:
        if str(layer_name).__contains__('conv'):
            if(layernamecopy=='module.conv1.weight'):
                activation_name = layer_name.replace('conv1', 'relu')
            else:
                activation_name = layer_name.replace('conv', 'relu')
            schedule.write("        " + activation_name + ":\n")
...
def run_distiller_summary():
    os.system(
        "python3 compress_classifier.py -a plain20_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/  --epochs 1 --resume  " + RESUME_PATH + "  --summary sparsity")
...
def run_distiller_quant():
    ...
    os.system(
        "python3 compress_classifier.py -a plain20_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume " + checkpoint_path + "/checkpoint.pth.tar --epochs 1 --effective-train-size " + str(
...
def run_distiller():
    ...
    os.system(
        "python3 compress_classifier.py -a plain20_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0000001 --out-dir logs/agp75/ --compress=schedules/temp.yaml --resume " + RESUME_PATH + " --epochs 1 --effective-train-size " + str(
            train_size) + " --effective-valid-size " + str(valid_size) + " --effective-test-size " + str(
...
def run_distiller_final():
    os.system(
        "python3 compress_classifier.py -a plain20_cifar "+ DATASET_PATH + " -p 30 -j=4 --lr=0.001 --out-dir logs/agp75/ --compress=schedules/final.yaml --resume  " + RESUME_PATH + "  --epochs " + str(
            PRUNE_EPOCHS))
...
def run_distiller_final_quant():
    checkpoint_path = read_checkpoint_path()
    os.system(
        "python3 compress_classifier.py -a plain20_cifar " + DATASET_PATH + " -p 30 -j=4 --lr=0.0008 --out-dir logs/agp75/ --compress=schedules/final_quant.yaml --resume " + checkpoint_path + "/checkpoint.pth.tar --epochs " + str(
